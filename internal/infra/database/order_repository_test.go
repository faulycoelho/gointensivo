...
func (suite *OrderRepositoryTestSuite) SetupSuite() {
	db, err := sql.Open("sqllite3", ":memory:")
	suite.NoError(err)
	db.Exec("create table orders(id varchar(255) not null primary key, price float not null, tax flot not null, final_price float not null)")
	suite.Db = db
}
